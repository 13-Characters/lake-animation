...
def get_sun_color(sun_alt):
  daytime = ((255, 255, 254), (255, 255, 255))
...
def place_sun(x, y, sun_color, sun_glow_color):
  ...
  sun_color = list(map(lambda x: str(round(x)), sun_color))
  sun_glow_color = list(map(lambda x: str(round(x)), sun_glow_color))
  for i, element in enumerate(sun_copy.childNodes):
    if type(element) != xml.dom.minidom.Element:
      continue
    if i < 4:
      element.setAttribute("style", f"display:inline;opacity:1;fill:rgb({' '.join(sun_glow_color)});fill-opacity:0.0538922;stroke:none;")
    else:
      element.setAttribute("style", f"display:inline;fill:rgb({' '.join(sun_color)});fill-opacity:1;stroke:none;")
...
def update_sky_colors(sun_azimuth, sun_altitude):
  ...
  if (sun_altitude > 0):
    for i, part in enumerate(["top", "bottom"]):
      ...
      sky_color = list(map(lambda x: str(round(x)), sky_color))
      stops[i].setAttribute("style", f"stop-color:rgb({' '.join(sky_color)});stop-opacity:1;")
